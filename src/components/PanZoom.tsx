...
export class PanZoom extends React.Component<any, any> {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (shouldPan && ready) {
      let promise;
      if (panCoordinates) promise = this.panToCoordinates(panCoordinates);
      else promise = this.panToEntities(pannedEntities);

      promise.then(() => this.setState({ shouldPan: false }));
    } else if (shouldZoom && ready) {
      let promise;
      if (zoomLevel) promise = this.zoomToLevel(zoomLevel);
      else promise = this.zoomOnEntities(zoomedEntities);
  ...
  handleChange = () => {
    ...
    if (onChange) onChange({ x: relX, y: relY, zoomLevel });
  };
  ...
  locate(entities: string[]) {
    ...
    const { panZoom } = this.state;
    const diagramDOMNode: SVGSVGElement = ReactDOM.findDOMNode(
      diagram
    ) as SVGSVGElement;
    ...
    const BBoxes = entities
      .map(
        singleEntity =>
          diagramDOMNode.querySelector(`g#${singleEntity}`) as SVGSVGElement
      )
      .filter(locatedEntity => !!locatedEntity)
      .map(locatedEntity =>
        Object.assign(
          {},
          { BBox: locatedEntity.getBBox(), matrix: locatedEntity.getCTM() }
        )
      )
      .map(coords => {
