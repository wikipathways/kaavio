...
module.exports = function() {
  ...
  function expandEntityReference(metadata, entityReference) {
    ...
    if (id) {
      // Don't show an active link if the db was never defined
      if (id.indexOf('identifiers.org/undefined') === -1) {
        entityReference['@id'] = id;
        dbFallback = 'More information';
      } else {
        dbFallback = 'Identifier/Label';
      }
    }
    var db = metadata.db || dbFallback;
    entityReferenceFallbackData.isDataItemIn.name = [db];
    _.defaults(entityReference, entityReferenceFallbackData);
  ...
  function getPreloadedData(metadata, entityReference) {
    ...
    preloadedData.listItems = [
      {
        key: entityReference.isDataItemIn.name[0],
  ...
  function render(kaavio, args) {
    ...
    if (entityReference.id.indexOf('identifiers.org') > -1 ||
      ...
      var bridgeDb = new BridgeDb({
        baseIri: 'http://pointer.ucsf.edu/d3/r/data-sources/bridgedb.php1/',
      ...
      highland(bridgeDb.xref.get(bridgedbArgs))
