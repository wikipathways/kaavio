...
function DiagramComponent(kaavio) {
  ...
  //* TODO standardize on either this mithril-style selectedElement.id()
  ...
  diagramComponent.vm = (function() {
    ...
    vm.init = function() {
      ...
      vm.selectedPvjsElements = vm.clickTargetIdSource
        .filter(function(id) {
          return !!id;
        })
        .map(function(id) {
          ...
          vm.selectedElement = new diagramComponent.SelectedElement(id);
          return id;
        })
      ...
      vm.selectedPvjsElements.filter(function(pvjsElement) {
          return !pvjsElement;
        })
        .forEach(function(nothing) {
          console.log('nothing');
          console.log(nothing);
          vm.reset();
        });
      ...
      vm.clickTargetTagName = m.prop('None');
      vm.clickTargetTagNameSource.subscribe(
          function(x) {
            console.log('Next:');
            console.log(x);
            vm.clickTargetTagName(x);
          },
          function(err) {
            console.log('Error: ' + err);
          },
  ...
  diagramComponent.view = function() {
    ...
    /*
    m('button', {
      onclick: rxWithAttr.call(vm, 'tagName')
    })
    //*/
    ];
    //});
