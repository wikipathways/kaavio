...
footer.vm = (function() {
  ...
  vm.init = function(ctrl) {
    ...
    placeholderContent.view = function(ctrl, args) {
      return m('div', {}, 'Placeholder' + new Date().toISOString());
    };
...
// placeholder
//footer.content = m.prop(m('div', {}, 'Placeholder'));
...
footer.view = function(ctrl, args) {
  ...
  if (footer.vm.state() === 'closed') {
    return m.component(args.footerOpenButton, args);
  } else if (footer.vm.state() === 'open') {
    return m.component(ctrl.vm.content, args);
    /*
    return m('section.kaavio-footer.kaavio-footer-open', {}, [
      m.component(footer.content, kaavio)
    ]);
    //*/
  }
};
