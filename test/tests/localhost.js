...
var lastKnownGoodScreenshotHashesPath =
  __dirname + "/../../test/last-known-goods/protocol/screenshot-hashes.json";
...
function getScreenshotHash(actualImagePath) {
  ...
  var screenshotJpgFilePath = actualImagePath.replace(".png", ".jpg");
...
function saveScreenshotHashes(
  ...
  console.log(
    "You said the screenshot looked OK, " +
  ...
  var updatedScreenshotHashes = JSON.stringify(
    lastKnownGoodScreenshotHashes,
    null,
    "\t"
  );
        ...
        .flatMap(function(result) {
          ...
          return notification
            .createStream({
              ...
              message:
                "Click here if it rendered correctly in " + browserName + ".",
            ...
            .flatMap(function(res) {
              if (res.indexOf("Activate") === -1) {
                var message =
                  "Expected screenshot not available for pathway " +
                  'named "' +
                  pathwayName +
              ...
              var actualImageStream = highland(
                fs.createReadStream(pathsToImages.actualImage)
              );
              ...
              return actualImageStream.fork().last();
            });
        })
        .flatMap(function() {
          ...
          if (String(screenshotHashActual) === String(screenshotHashExpected)) {
            ...
            .flatMap(function(imagesAreSame) {
              ...
              highland(_.range(5))
                .map(function(iteration) {
                  browser
                    .get("file://" + pathsToImages.diffImage)
                    .sleep(sleepPeriod)
              ...
              return notification
                ...
                .flatMap(function(res) {
                  if (res.indexOf("Activate") === -1) {
                    var message =
                      "Hash of expected screenshot did not match " +
