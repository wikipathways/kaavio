...
function testElementCount(pathwayName, elementName, element, selector) {
  ...
  if (expectedCount > 0) {
    ptor.findElements(protractor.By.css(selector)).then(function(arr) {
      var length = arr.length; //length
      describe(pathwayName + " test protocol pathway", function() {
        it(
          "should have " + expectedCount + " " + elementName + "(s)",
          function() {
...
function testPathway(pathwayName) {
  ...
  ptor
    ...
    .then(function() {
      return ptor.wait(
        forElementToBePresent(by.css("#viewport .info-box")),
        30 * 1000
      );
    })
    .then(function() {
      ...
      testElementCount(
        ...
        "gpml:Anchor",
        element,
        '#viewport [typeof~="gpml:Anchor"]'
      );
      testElementCount(
        pathwayName,
        "gpml:DataNode",
        element,
        '#viewport [typeof~="gpml:DataNode"]'
      );
      testElementCount(
        pathwayName,
        "gpml:GeneProduct",
        element,
        '#viewport [typeof~="gpml:GeneProduct"]'
      );
      testElementCount(
        pathwayName,
        "gpml:Metabolite",
        element,
        '#viewport [typeof~="gpml:Metabolite"]'
      );
      testElementCount(
        pathwayName,
        "gpml:Pathway",
        element,
        '#viewport [typeof~="gpml:DataNode"] + [typeof~="gpml:Pathway"]'
      );
      testElementCount(
        pathwayName,
        "gpml:Protein",
        element,
        '#viewport [typeof~="gpml:Protein"]'
      );
      testElementCount(
        pathwayName,
        "gpml:Rna",
        element,
        '#viewport [typeof~="gpml:Rna"]'
      );
      testElementCount(
        pathwayName,
        "gpml:Unknown",
        element,
        '#viewport [typeof~="gpml:Unknown"]'
      );
      testElementCount(
        pathwayName,
        "gpml:Group",
        element,
        '#viewport [typeof~="gpml:Group"]'
      );
      ...
      testElementCount(
        ...
        "gpml:GraphicalLine",
        element,
        '#viewport [typeof~="gpml:GraphicalLine"]'
      );
      testElementCount(
        pathwayName,
        "gpml:Interaction",
        element,
        '#viewport [typeof~="gpml:Interaction"]'
      );
    });
}
