...
var bodyElements,
  shapes,
  loadingIconBeforePathwayLoaded,
  loadingIconAfterPathwayLoaded,
  shapesInShapes;
...
ptor
  .get(baseUri + "test/development.html?gpml=WP2545")
  .then(function() {
    console.log("************** running anchors rendering test protocol...");
    return ptor.wait(
      forElementToBePresent(by.css("#pathvisiojs-is-loaded")),
      30 * 1000
    );
  })
  .then(function() {
    var expectedCount = 1;
    expect(element.all(by.css("#viewport .info-box")).count()).toEqual(
      expectedCount
    );
  ...
  .then(function() {
    ...
    expect(element.all(by.css("#viewport .node")).count()).toEqual(
      expectedCount
    );
    return "success";
  })
  .then(function() {
    ...
    expect(element.all(by.css("#viewport .anchor")).count()).toEqual(
      expectedCount
    );
    return "success";
  })
  .then(function() {
    ...
    expect(element.all(by.css("#viewport .data-node")).count()).toEqual(
      expectedCount
    );
    return "success";
  })
  .then(function() {
    var expectedCount = 1;
    expect(element.all(by.css("#viewport .gene-product")).count()).toEqual(
      expectedCount
    );
    testTheCount(
      "anchors test protocol pathway",
      "gene-product",
      expectedCount
    );
    return "sucess";
  ...
  .then(function() {
    var expectedCount = 2;
    expect(element.all(by.css("#viewport .metabolite")).count()).toEqual(
      expectedCount
    );
  ...
  .then(function() {
    var expectedCount = 1;
    expect(element.all(by.css("#viewport .data-node.pathway")).count()).toEqual(
      expectedCount
    );
    testTheCount(
      "anchors test protocol pathway",
      "pathway (as data-node)",
      expectedCount
    );
    return "sucess";
  })
  .then(function() {
    var expectedCount = 0;
    expect(element.all(by.css("#viewport .protein")).count()).toEqual(
      expectedCount
    );
  ...
  .then(function() {
    var expectedCount = 0;
    expect(element.all(by.css("#viewport .rna")).count()).toEqual(
      expectedCount
    );
  ...
  .then(function() {
    var expectedCount = 0;
    expect(element.all(by.css("#viewport .unknown")).count()).toEqual(
      expectedCount
    );
  ...
  .then(function() {
    var expectedCount = 1;
    expect(element.all(by.css("#viewport .group-node")).count()).toEqual(
      expectedCount
    );
  ...
  .then(function() {
    var expectedCount = 9;
    expect(element.all(by.css("#viewport .edge")).count()).toEqual(
      expectedCount
    );
  ...
  .then(function() {
    var expectedCount = 9;
    expect(element.all(by.css("#viewport .interaction")).count()).toEqual(
      expectedCount
    );
    testTheCount(
      "anchors test protocol pathway",
  ...
  .then(function() {
    ...
    var expectedCount = 5;
    expect(element.all(by.css("#viewport .citation")).count()).toEqual(
      expectedCount
    );
    testTheCount(
      "citations test protocol pathway",
      "element-specific citation list string",
      expectedCount
    );
    return "success";
  })
  ...
  .then(function() {
    console.log(
      "************** running interactions rendering test protocol..."
    );
  ...
  .then(function() {
    ...
    testTheCount(
      "groups test protocol pathway",
      "groups, also called group-node",
      expectedCount
    );
    return "success";
  })
  .then(function() {
    var expectedCount = 4;
    expect(element.all(by.css("#viewport .complex")).count()).toEqual(
      expectedCount
    );
    testTheCount(
      "groups test protocol pathway",
      'groups of type "complex"',
      expectedCount
    );
    return "success";
  })
  .then(function() {
    var expectedCount = 1;
    expect(element.all(by.css("#viewport .group-node.group")).count()).toEqual(
      expectedCount
    );
    testTheCount(
      "groups test protocol pathway",
  ...
  .then(function() {
    var expectedCount = 2;
    expect(
      element.all(by.css("#viewport .group-node.pathway")).count()
    ).toEqual(expectedCount);
    testTheCount(
      "groups test protocol pathway",
      'groups of type "pathway"',
      expectedCount
    );
    return "success";
  })
  .then(function() {
    var expectedCount = 1;
    expect(element.all(by.css("#viewport .group-node.none")).count()).toEqual(
      expectedCount
    );
    testTheCount(
      "groups test protocol pathway",
