...
(function(process) {
  ...
  RDFa.attach = function(document, update) {
    ...
    RDFaProcessor.prototype.process = function(node) {
      ...
      queue.push({
        current: node,
        context: this.push(null, node.baseURI)
      });
      while (queue.length > 0) {
        ...
        if (item.parent) {
          ...
          for (var predicate in item.listMapping) {
            ...
            if (list.length == 0) {
              this.addTriple(this.target, item.parent, item.subject, predicate, {
                type: this.objectURI,
            ...
            for (var i = 0; i < bnodes.length; i++) {
              ...
              this.addTriple(this.target, item.parent, bnodes[i], "http://www.w3.org/1999/02/22-rdf-syntax-ns#rest", {
                type: this.objectURI,
        ...
        if (vocabAtt) {
          ...
          if (value.length > 0) {
            ...
            this.addTriple(this.target, current, baseSubject, "http://www.w3.org/ns/rdfa#usesVocabulary", {
              type: this.objectURI,
              value: vocabulary
            });
          } else {
        ...
        if (typedResource) {
          ...
          for (var i = 0; i < values.length; i++) {
            ...
            if (object) {
              this.addTriple(this.target, current, typedResource, this.typeURI, {
                type: this.objectURI,
                value: object
              });
            }
        ...
        if (currentObjectResource) {
          if (relAtt && inlistAtt) {
            ...
            for (var i = 0; i < values.length; i++) {
              var predicate = this.parsePredicate(values[i], vocabulary, context.terms, prefixes, base);
              if (predicate) {
                this.addTriple(this.target, current, newSubject, predicate, {
                  type: this.objectURI,
                  value: currentObjectResource
                });
              }
          ...
          if (revAtt) {
            ...
            for (var i = 0; i < values.length; i++) {
              var predicate = this.parsePredicate(values[i], vocabulary, context.terms, prefixes, base);
              if (predicate) {
                this.addTriple(this.target, current, currentObjectResource, predicate, {
                  type: this.objectURI,
                  value: newSubject
                });
              }
          ...
          for (var i = 0; i < values.length; i++) {
            ...
            if (predicate) {
              if (inlistAtt) {
                ...
                list.push(datatype == this.XMLLiteralURI ? {
                  type: this.XMLLiteralURI,
                  value: current.childNodes
                } : {
                  type: datatype ? datatype : this.PlainLiteralURI,
                  value: content,
                  language: language
                });
              } else {
        ...
        if (newSubject && !skip) {
          for (var i = 0; i < context.incomplete.length; i++) {
            if (context.incomplete[i].list) {
              ...
              this.addTriple(this.target, current, context.subject, context.incomplete[i].predicate, {
                type: this.objectURI,
                value: newSubject
              });
            } else {
              this.addTriple(this.target, current, newSubject, context.incomplete[i].predicate, {
                type: this.objectURI,
