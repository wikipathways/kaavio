...
function HtmlToDom(parser) {
  if (parser && parser.write) {
    ...
    if (parser.ParseHtml) {} else if (parser.DefaultHandler) {
...
function setChild(parent, node) {
  ...
  switch (node.type) {
    ...
    case 'style':
      ...
      break;
    case 'text':
      newNode = currentDocument.createTextNode(/^(?:script|style)$/i.test(parent.nodeName) ? node.data : HTMLDecode(node.data));
      break;
    case 'comment':
