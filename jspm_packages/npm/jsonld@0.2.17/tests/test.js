...
(function(process) {
  (function() {
    ...
    if (_nodejs) {
      ...
      program.option('--earl [filename]', 'Output an earl report').option('--bail', 'Bail when a test fails').parse(process.argv);
    } else {
      var fs = require("fs");
    ...
    var ROOT_MANIFEST_DIR = resolvePath(getEnv().JSONLD_TEST_SUITE || JSONLD_TEST_SUITE);
    var TEST_TYPES = {
      'jld:CompactTest': {
        fn: 'compact',
        params: [readTestUrl('input'), readTestJson('context'), createTestOptions()],
        compare: compareExpectedJson
      },
      'jld:ExpandTest': {
        fn: 'expand',
        params: [readTestUrl('input'), createTestOptions()],
        compare: compareExpectedJson
      },
      'jld:FlattenTest': {
        fn: 'flatten',
      ...
      'jld:FrameTest': {
        fn: 'frame',
        params: [readTestUrl('input'), readTestJson('frame'), createTestOptions()],
        compare: compareExpectedJson
      },
      'jld:FromRDFTest': {
        fn: 'fromRDF',
        params: [readTestNQuads('input'), createTestOptions({format: 'application/nquads'})],
        compare: compareExpectedJson
      },
      'jld:NormalizeTest': {
        fn: 'normalize',
        params: [readTestUrl('input'), createTestOptions({format: 'application/nquads'})],
    ...
    describe('JSON-LD', function() {
      ...
      if (!_nodejs) {
        require("./webidl/testharness");
        require("./webidl/WebIDLParser");
        require("./webidl/idlharness");
        describe('Web IDL', function() {
          add_result_callback(function(test) {
            it(test.name, function(done) {
              var msg = test.message || '';
              if (msg.indexOf('JsonLdProcessor.prototype is writable expected false') !== -1) {
                test.status = 0;
              }
              if (msg.indexOf('"JsonLdProcessor" is enumerable expected false') !== -1) {
                test.status = 0;
              }
    ...
    function addTest(manifest, test) {
      ...
      if (!isJsonLdType(test, testTypes) || isJsonLdType(test, SKIP_TESTS)) {
        console.log('Skipping test "' + test.name + '" of type: ' + getJsonLdValues(test, '@type'));
      }
    ...
    function createTestOptions(opts) {
      return function(test) {
        var options = {documentLoader: createDocumentLoader(test)};
    ...
    function joinPath() {
      return (_nodejs ? path : fs).join.apply(null, Array.prototype.slice.call(arguments));
    }
      ...
      function loadLocally(url) {
        ...
        if (options && url === test.base) {
          if ('redirectTo' in options && parseInt(options.httpStatus, 10) >= 300) {
            ...
            linkHeader = jsonld.parseLinkHeader(linkHeader)['http://www.w3.org/ns/json-ld#context'];
        ...
        var filename = joinPath(ROOT_MANIFEST_DIR, doc.documentUrl.substr(base.length));
        try {
