...
(function(process) {
  ...
  describe('BridgeDb.Dataset.get', function() {
    ...
    it('should get metadata by provider @id', function(done) {
      lkgDataPath = __dirname + '/entrez-gene-dataset-metadata.jsonld';
      ...
      bridgeDbInstance.dataset.get({'@id': 'http://www.ncbi.nlm.nih.gov/gene/'}).map(function(data) {
    ...
    it('should get metadata by identifiers.org @id', function(done) {
      ...
      bridgeDbInstance.dataset.get({'@id': 'http://identifiers.org/ncbigene'}).map(function(data) {
        return JSON.stringify(data);
      }).pipe(highland.pipeline(function(s) {
        if (update) {
          s.fork().map(function(dataString) {
