...
(function(process) {
  window.CodeMirror = (function() {
    ...
    function updateDisplayInner(cm, changes, viewPort) {
      ...
      if (changes !== true && changes.length == 0 && visible.from > display.showingFrom && visible.to < display.showingTo)
        return ;
      ...
      for (var node = display.lineDiv.firstChild,
          height; node; node = node.nextSibling)
        if (node.lineObj) {
          if (ie_lt8) {
    ...
    function buildLineElement(cm, line, lineNo, dims) {
      ...
      var markers = line.gutterMarkers,
          display = cm.display;
      if (!cm.options.lineNumbers && !markers && !line.bgClass && !line.wrapClass && (!line.widgets || !line.widgets.length))
        return lineElement;
      var wrap = elt("div", null, line.wrapClass, "position: relative");
      if (cm.options.lineNumbers || markers) {
        var gutterWrap = wrap.appendChild(elt("div", null, null, "position: absolute; left: " + dims.fixedPos + "px"));
    ...
    function measureLine(cm, line) {
      ...
      for (var i = 0; i < cache.length; ++i) {
        var memo = cache[i];
        if (memo.text == line.text && memo.markedSpans == line.markedSpans && display.scroller.clientWidth == memo.width)
          return memo.measure;
      }
    ...
    function measureLineInner(cm, line) {
      ...
      if (ie && !ie_lt8 && !cm.options.lineWrapping && pre.childNodes.length > 100) {
        ...
        for (var i = 0,
            chunks = Math.ceil(n / chunk); i < chunks; ++i) {
    ...
    function readInput(cm) {
      ...
      updateDoc(cm, from, to, splitLines(text.slice(same)), "end", cm.display.pasteIncoming ? "paste" : "input", {
        from: from,
        to: to
      });
      ...
      if (wheelSamples < 20) {
        if (wheelStartX == null) {
          ...
          setTimeout(function() {
            ...
            var sample = (movedY && wheelDY && movedY / wheelDY) || (movedX && wheelDX && movedX / wheelDX);
    ...
    function onKeyPress(e) {
      ...
      if (this.options.electricChars && this.view.mode.electricChars && this.options.smartIndent && !isReadOnly(this) && this.view.mode.electricChars.indexOf(ch) > -1)
        setTimeout(operation(cm, function() {
    ...
    function updateDocInner(cm, from, to, newText, selUpdate, origin) {
      ...
      if (view.history)
        addChange(cm, from.line, newText.length, old, origin, startSelFrom, startSelTo, view.sel.from, view.sel.to);
      return retval;
    }
    ...
    CodeMirror.prototype = {
      ...
      markText: operation(null, function(from, to, options) {
        return markText(this, clipPos(this.view.doc, from), clipPos(this.view.doc, to), options, "range");
      }),
    ...
    var commands = CodeMirror.commands = {
      ...
      killLine: function(cm) {
        ...
        if (!sel && cm.getLine(from.line).length == from.ch)
          ...
          cm.replaceRange("", from, sel ? to : {line: from.line}, "delete");
      },
    ...
    function lineIsHiddenInner(line, span) {
      ...
      for (var sp,
        ...
        if (sp.marker.collapsed && sp.from == span.to && (sp.marker.inclusiveLeft || span.marker.inclusiveRight) && lineIsHiddenInner(line, sp))
          return true;
      }
    }
    function hlText(val) {
    ...
    function buildTokenMeasure(builder, text, style, startStyle, endStyle) {
      for (var i = 0; i < text.length; ++i) {
        if (i && i < text.length - 1 && builder.cm.options.lineWrapping && spanAffectsWrapping.test(text.slice(i - 1, i + 1)))
          builder.pre.appendChild(elt("wbr"));
        builder.measure[builder.pos++] = buildToken(builder, text.charAt(i), style, i == 0 && startStyle, i == text.length - 1 && endStyle);
      }
      if (text.length)
        builder.addedOne = true;
    }
        ...
        while (true) {
          if (text) {
            ...
            if (!collapsed) {
              ...
              builder.addToken(builder, tokenText, style + spanStyle, spanStartStyle, pos + tokenText.length == nextChange ? spanEndStyle : "");
            }
            if (end >= upto) {
    ...
    function signalLater(cm, emitter, type) {
      var arr = emitter._handlers && emitter._handlers[type];
      if (!arr)
        return ;
