...
function CSVStream(opts, cb) {
  ...
  if (opts.inputEncoding) {
    if (!iconv)
      throw new Error('Please "npm install iconv-lite".');
...
CSVStream.prototype.parse = function(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (c !== this.quote && this._prev[0] === this.quote && this._prev[1] !== this.quote) {
      this._isQuoted = this._isQuoted ? false : true;
    }
    if (c !== this.quote && this._prev[0] === this.quote && this._prev[1] === this.quote && this._prev[2] === this.quote) {
      this._isQuoted = this._isQuoted ? false : true;
      this._field += this.quote;
    }
    if (this._field && c !== this.quote && this._prev[0] === this.quote && this._prev[1] === this.quote && this._prev[2] !== this.quote) {
      this._field += this.quote;
    }
    if (!this._isQuoted && c === this.delimiter) {
      if (this._field === '')
        this._field = this.empty;
