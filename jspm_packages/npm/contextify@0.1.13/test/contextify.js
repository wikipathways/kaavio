...
exports['basic tests'] = {
  ...
  'test accessors on sandbox': function(test) {
    var sandbox = {};
    sandbox.__defineGetter__('test', function() {
      return 3;
    });
    sandbox.__defineSetter__('test2', function(val) {
      this.x = val;
    });
    Contextify(sandbox);
...
exports['synchronous script tests'] = {
  ...
  'sandbox properties should be globals': function(test) {
    ...
    Contextify(sandbox);
    sandbox.run("test1 = (prop1 == 'prop1');" + "test2 = (prop2 == 'prop2');");
...
exports['asynchronous script tests'] = {
  ...
  'sandbox properties should be globals': function(test) {
    ...
    Contextify(sandbox);
    sandbox.run("setTimeout(function () {" + "test1 = (prop1 == 'prop1');" + "test2 = (prop2 == 'prop2');" + "}, 0)");
  ...
  'createContext: sandbox properties should be globals': function(test) {
    ...
    var context = Contextify.createContext(sandbox);
    context.run("setTimeout(function () {" + "test1 = (prop1 == 'prop1');" + "test2 = (prop2 == 'prop2');" + "}, 0)");
    test.equal(sandbox.test1, undefined);
    test.equal(sandbox.test2, undefined);
    setTimeout(function() {
      test.ok(sandbox.test1);
  ...
  'setTimeout should not fail after dispose': function(test) {
    ...
    Contextify(sandbox);
    sandbox.run("setTimeout(function () {" + "test.ok(prop1 == 'prop1');" + "test.ok(prop2 == 'prop2');" + "test.done();" + "}, 10)");
    test.equal(sandbox.test1, undefined);
    test.equal(sandbox.test2, undefined);
    sandbox.dispose();
  }
...
exports['test global'] = {
  ...
  'self references to the global object': function(test) {
    ...
    sandbox.run("test1 = (this == ref1);" + "test2 = (this == ref2.ref2);");
    test.ok(sandbox.test1);
    test.ok(sandbox.test2);
    test.done();
  },
